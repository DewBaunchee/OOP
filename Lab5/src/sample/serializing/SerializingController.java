...
public class SerializingController {
    ...
    @FXML
    private RadioButton jsonMethod;
        ...
        serializeBtn.setOnAction(actionEvent -> {
            ...
            if (xmlMethod.isSelected()) {
                try {
                    ...
                    byte[] xml = Legion.getLegion().xmlSerialize();
                ...
                try {
                    ...
                    Legion.getLegion().writeExternal(new ObjectOutputStream(byteArrayOutputStream));
                    byte[] serialized = byteArrayOutputStream.toByteArray();

                    if (useCryptCheck.isSelected())
                        serialized = loadedPlugin.encrypt(serialized, keyField.getText().getBytes());
            ...
            } else if (jsonMethod.isSelected()) {
                try {
                    byte[] header = (loadedPluginClass + "|" + "json" + "|").getBytes();
                    IJSONAdapter jsonAdapter = new JSONAdapter(Legion.getLegion());
                    byte[] serialized = jsonAdapter.toJson();
